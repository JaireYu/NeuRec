...
class FISM(AbstractRecommender):
    ...
    def train_model(self):
        ...
        for epoch in range(1, self.num_epochs+1):
            ...
            if self.is_pairwise is True:
                ...
                for index, (bat_users, bat_idx, bat_items, bat_labels) in enumerate(data_iter):
    ...
    @timer
    def evaluate(self):
        self.cur_embedding_Q_, self.cur_embedding_Q, self.cur_bias \
            = self.sess.run([self.embedding_Q_, self.embedding_Q, self.bias])
    ...
    def predict(self, user_ids, candidate_items_userids):      
        ...
        if candidate_items_userids is not None:
            ...
            for u in user_ids:
                ...
                if (True):
                    print("u_id:" + str(u))
                for i in range(int(self.num_items/5000) + 1):
                    feed_dict = {self.user_input: user_input[i*5000: i*5000+5000],
                                 self.num_idx: item_idx[i*5000: i*5000+5000],
                                 self.item_input: eval_items[i*5000: i*5000+5000]}
                    ratings_row.extend(self.sess.run(self.output, feed_dict=feed_dict))
                a = self.sess.run(self.output, feed_dict=feed_dict)
                assert np.sum(a != a) == 0
                ratings.append(ratings_row)
                """
                embedding_p = np.sum(self.cur_embedding_Q_[cand_items_by_u], axis = 0)
                #embedding_p = np.tile(embedding_p, (self.num_items, 1))
                coeff = np.power(num_idx, self.alpha)
                output = coeff * np.sum(np.multiply(embedding_p, self.cur_embedding_Q), axis=1) + self.cur_bias
