...
class Dataset(object):
    ...
    def _check_saved_data(self, splitter, ori_prefix, saved_prefix):
        ...
        if splitter in ("loo", "ratio"):
            ...
            val_file = ori_prefix + ".val"
            test_file = ori_prefix + ".test"
            ori_file_md5 = [check_md5(file) for file in [train_file, val_file, test_file]]
        else:
        ...
        # check saved files
        for postfix in [".train", ".val", ".test", ".user2id", ".item2id"]:
    ...
    def _load_data(self, config):
        ...
        if self._check_saved_data(splitter, ori_prefix, saved_prefix):
            ...
            val_data = pd.read_csv(val_file, sep=sep, header=None, names=columns)
            ...
            train_data, val_data, test_data = self._split_data(ori_prefix, saved_prefix, columns, by_time, config)
        ...
        self.negative_matrix = self._load_test_neg_items(all_data, config, ori_prefix, saved_prefix)
    ...
    def _split_data(self, ori_prefix, saved_prefix, columns, by_time, config):
        ...
        if splitter in ("loo", "ratio"):
            ...
            with open(saved_prefix+".md5", "w") as md5_out:
                ...
                md5_out.writelines(check_md5(val_file))
        ...
        val_data["user"] = val_data["user"].map(self.userids)
        ...
        all_remapped_data = pd.concat([train_data, val_data, test_data])
    ...
    def _load_test_neg_items(self, all_data, config, ori_prefix, saved_prefix):
        number_neg = config["rec.evaluate.neg"]
        sep = config["data.convert.separator"]
        neg_matrix = None
        if number_neg < 0:
            neg_items_file = ori_prefix + '.expose'
            user_list, item_list = [], []
            with open(neg_items_file) as f:
                for line in f.readlines():
                    line = line.strip().split(sep)
                    item_list.extend([self.itemids[int(i)] for i in line[1:]])
                    user_list.extend([self.userids[int(line[0])]] * len(line[1:]))
    ...
    def get_user_val_dict(self):
        test_dict = csr_to_user_dict(self.val_matrix)
        return test_dict
    ...
    def get_user_val_neg_dict(self):
        val_neg_dict = None
        if self.negative_matrix is not None:
            val_neg_dict = csr_to_user_dict(self.negative_matrix)
        return val_neg_dict
