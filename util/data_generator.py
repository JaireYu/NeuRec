...
def _get_pointwise_all_likefism_data_debug(dataset, num_negatives, train_dict):
    user_input,num_idx,item_input,labels = [],[],[],[]
    num_users = dataset.num_users
    num_items = dataset.num_items
    for u in range(num_users):
...
def _get_pointwise_all_likefism_data_debug_fast(dataset, num_negatives, train_dict):
    user_input,num_idx,item_input,labels = [],[],[],[]
    num_users = dataset.num_users
    num_items = dataset.num_items
    indices = []
    ...
    for i in range(num_users):
        indices.append(len(train_dict[i]))
        rating += len(train_dict[i])
    indices = np.argsort(np.array(indices))
    shrehold = 128*600
    last_point = 0
    batch_point = [0]
    cnt = 0
    for index_debug, u in enumerate(indices):
        items_by_user = train_dict[u].copy()
        items_set = set(items_by_user)
        size = len(items_by_user)
        for i in items_by_user:
            ...
            labels.append(1)
            items_by_user.insert(index, i)
            if (cnt - last_point) * len(train_dict[u]) > shrehold:
                batch_point.append(cnt*5)
                last_point = cnt
            cnt += 1
    batch_point.append(len(user_input))
    return user_input,num_idx,item_input,labels, batch_point
